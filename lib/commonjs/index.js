...
var _UpsellBackgroundActionsModule = require("./UpsellBackgroundActionsModule.js");
...
class BackgroundServer extends _eventemitter.default {
   ...
   */
  _addListeners() {
    _UpsellBackgroundActionsModule.nativeEventEmitter.addListener('expiration', () => this.emit('expiration'));
  }
  ...
  async updateNotification(taskData) {
    ...
    await _UpsellBackgroundActionsModule.UpsellBackgroundActions.updateNotification(this._currentOptions);
  }
  ...
  async setCallBack(callback) {
    _UpsellBackgroundActionsModule.UpsellBackgroundActions.setCallBack(callback);
  }
  ...
  async start(task, options, triggerTime) {
    ...
    if (_reactNative.Platform.OS === 'android') {
      ...
      await _UpsellBackgroundActionsModule.UpsellBackgroundActions.start(this._currentOptions, triggerTime);
  ...
  async stop() {
    this._stopTask();
    await _UpsellBackgroundActionsModule.UpsellBackgroundActions.stop();
    this._isRunning = false;
  }
  async sendStopBroadcast() {
    this._stopTask();
    await _UpsellBackgroundActionsModule.UpsellBackgroundActions.sendStopBroadcast();
    this._isRunning = false;
  }
  async isBackgroundServiceRunning() {
    return await _UpsellBackgroundActionsModule.UpsellBackgroundActions.isBackgroundServiceRunning();
  }
  async checkScheduleExactAlarmPermission() {
    return await _UpsellBackgroundActionsModule.UpsellBackgroundActions.checkScheduleExactAlarmPermission();
  }
  async requestExactAlarmPermission() {
    return await _UpsellBackgroundActionsModule.UpsellBackgroundActions.requestExactAlarmPermission();
  }
  async listRunningServices() {
    return await _UpsellBackgroundActionsModule.UpsellBackgroundActions.listRunningServices();
  }
  async stopAlarm() {
    return await _UpsellBackgroundActionsModule.UpsellBackgroundActions.stopAlarm();
  }
}
